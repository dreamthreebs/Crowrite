...
本章首先回顾了**为什么需要暴胀**，核心动机来自两个经典宇宙学难题：

1. **视界问题（Horizon problem）**
   * CMB 在相距极远、在标准大爆炸模型下**因果不相连**的区域，呈现出几乎相同的温度；
   * 暴胀通过在早期引入一段**指数膨胀阶段**，使得这些区域在暴胀前实际上曾经处于因果接触之中。
2. **平坦性问题（Flatness problem）**
   * Friedmann 方程显示，(\Omega = 1) 是一个**不稳定固定点**；
...
* 暴胀发生在**极高能标**：\
  \[\
  V^{1/4} \sim 10^{16},\text{GeV} \quad (\text{典型估计})\
  ]
...
* 我们通过 **CMB 各向异性**间接探测这些极端能标；
* 暴胀是连接**宇宙学与高能物理**的关键桥梁。
...
这是一个极其大胆但成功的思想：\
\[\
\text{量子涨落} ;\longrightarrow; \text{CMB 各向异性} ;\longrightarrow; \text{宇宙结构}\
]
